...
| 功能 | 入口 | 说明 |
|---|---|---|
| 智能路由 | CLI / Skill / MCP | 自动识别 BTC、GC、CLUSDT、EURUSD、600519、AAPL 等标的所属市场 |
| 加密货币分析 | CLI / Skill / MCP | Binance、CoinGecko、链上、合约、情绪、宏观、期权、缠论；严格八维 |
| 商品/股指期货 | CLI / Skill / MCP | 八维分析；Yahoo/CFTC/EIA/新闻/代理资产 + Binance TradFi 永续 K线、资金费率、OI、多空比 + CZSC确认 |
| 外汇分析 | CLI / Skill / MCP | 八维分析；汇率、DXY、VIX、美国利率、对手国利率代理、宏观事件、CFTC + CZSC确认 |
| A股分析 | CLI / Skill / MCP | 八维分析；指数、个股、北向/广度/板块资金流，支持国内远程节点和本地降级 + CZSC确认 |
| 美股/ETF/指数 | CLI / Skill / MCP | 八维分析；日线/小时线、宏观代理、新闻、财报/监管/业务事件代理 + CZSC确认 |
...
| MCP prompts | MCP | `deep_market_analysis`、`eight_dimension_analysis`、`crypto_eight_dimension_analysis`、`czsc_confirmation_review` |
...
`--no-czsc` 只适合调试数据采集链路。正式市场分析不能使用 `--no-czsc`，否则第 8 维缺失，输出必须降级为“不足/不可用”。
...
防呆规则：任何市场请求都不能只输出快速行情摘要。客户端应拉取完整数据，尽量运行 CZSC，并输出完整八维、`各维度证据`、`方向判断依据`、`反向审计`、`缠论确认` 和 `最终方向`；`各维度证据` 必须先由 1-7 维得出，最终方向由 AI 基于证据逻辑强度综合判断（不用投票计数或权重打分），并通过反向审计。证据分裂、过期、缺失或被关键反证打脸时，最终方向必须是 `观望 / 震荡 / 无方向优势`。CZSC 只能确认/冲突/降级/细化执行，不能用 CZSC score 直接决定最终方向；CZSC 不可用时必须标注第 8 维不足。
...
| `eight_dimension_analysis` | 生成任意支持市场的严格八维分析流程提示 |
