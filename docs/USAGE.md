...
防呆规则：任何市场请求都不能只输出快速行情摘要。客户端应拉取完整数据，尽量运行 CZSC，并输出完整八维、`七维主判断`、`方向质量门槛`、`反向审计`、`缠论确认` 和 `最终方向`；`七维主判断` 必须先由 1-7 维得出，最终方向必须通过方向质量门槛和反向审计。证据分裂、过期、缺失或被关键反证打脸时，最终方向必须是 `观望 / 震荡 / 无方向优势`。CZSC 只能确认/冲突/降级/细化执行，不能用 CZSC score 直接决定最终方向；CZSC 不可用时必须标注第 8 维不足。
