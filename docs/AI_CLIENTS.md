...
为防止 AI 工具把市场分析误写成快速行情摘要，或把缠论分数当成最终交易方向，MCP server instructions、MCP prompts、`AGENTS.md` 和各客户端 rules 都明确要求：所有市场尽量走八维框架，拉取完整数据，先用 1-7 维输出 `七维主判断`，再输出 `方向质量门槛` 和 `反向审计`，之后运行 CZSC 或标注第 8 维不足，最后输出 `缠论确认`、`最终方向`。证据分裂、过期、缺失或被关键反证打脸时，最终方向必须是 `观望 / 震荡 / 无方向优势`。CZSC 只能确认、冲突、降级置信度或细化执行。
