...
- 只按独立维度计票，不能把 DXY/10Y/VIX/ETF 等同类代理拆成多票；只有至少 3 个独立维度同向、领先反向至少 2 个维度、关键维度无缺口、反向审计不构成同等级证据时，才允许输出做多/做空。
...
**强制输出规则**：
1. 先给 `七维主判断`，只基于第 1-7 维；必须写明主导因素、反证因素和权重，不允许先看 CZSC 再倒推理由。
2. 再给 `方向质量门槛`：列出偏多/偏空/中性/缺失维度，确认是否达到方向优势。
3. 必须做 `反向审计`：先写出最强反方向证据；若反方向证据接近或关键驱动冲突，最终方向降级为观望。
4. 再给 `缠论确认`，说明 CZSC 是确认、冲突还是不足。
5. 最终方向由 `七维主判断` 通过质量门槛后决定；第 8 维只能提高/降低置信度、细化入场止损或触发观望，不能用 CZSC score 覆盖 1-7 维。
6. 使用 `python3 -m hermes_finance analyze futures <SYMBOL>` 或 MCP `analyze_futures`，默认会尽量用采集器 K 线跑 CZSC。
