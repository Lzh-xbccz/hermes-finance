...
| # | 维度 | 期货对应项 | 核心关注 |
|---|------|-----------|---------|
| 1 | 📈 技术面 | K线趋势、量价关系、支撑阻力、形态识别 | 90D/30D/24H 结构、摆点序列 |
| 2 | ⛓️ 市场结构 | 期限结构、现货/近月/远月关系、库存与基差背景 | contango/backwardation、库存趋势、近远月价差 |
| 3 | 🐋 主导力量 | 波动率代理(OVX/VIX)、ETF 资金流(USO/GLD/SLV)、COT 滞后参考、DXY | 谁在定价？恐慌定价还是供需定价？ |
| 4 | 😱 情绪面 | OVX（原油）/ VIX（股指）/ 风险偏好 / 挤仓情绪 | 波动率是否在极端区间？恐慌/贪婪程度 |
| 5 | 🌍 宏观基本面 | 地缘、利率(^TNX)、美元(DXY)、库存(EIA)、政策(OPEC/Fed)、供需 | **权重最高**——地缘 > 库存 > 美元 > 结构 |
| 6 | 🔍 交叉验证 | 相关资产、美元指数、美债收益率、ETF、现货代理 | 关联市场是否确认方向？（单一交易所无多所对比） |
...
### 六维判断
- 📈 技术面：[结论 — 趋势方向 + 关键结构位]
- ⛓️ 市场结构：[结论 — contango/backwardation 倾向 + 库存背景]
- 🐋 主导力量：[结论 — OVX/VIX 状态 + ETF 资金流 + DXY 方向]
- 😱 情绪面：[结论 — 波动率区间 + 恐慌/贪婪判断]
- 🌍 宏观面：[结论 — 地缘/政策/库存/利率核心驱动]
- 🔍 交叉验证：[结论 — 关联资产是否确认 / 是否有背离]
