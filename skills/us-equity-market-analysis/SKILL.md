...
1. **先判断是否有方向优势**：个股/ETF→🟢做多/🔴做空/⚪观望；现金指数→偏多/偏空/震荡。只有技术、行业/ETF、公司事件、VIX、利率、交叉验证多数同向且反向审计不过关时，才允许给做多/做空；否则默认观望/震荡。
...
3. 必须做 `反向审计`：若公司事件、行业 ETF、大盘、VIX 或利率与技术方向冲突，最终方向降级为观望/震荡。
4. 再给 `缠论确认`，说明 CZSC 是确认、冲突还是不足。
5. 个股财报/监管事件优先级高于缠论；CZSC 不能覆盖事件风险。
6. 使用 `python3 -m hermes_finance analyze us-equity <TICKER>` 或 MCP `analyze_us_equity`，默认会尽量用采集器 K 线跑 CZSC。
