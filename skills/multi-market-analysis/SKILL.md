...
6. **⚠️ 数据获取通过各市场的 fetch 脚本（scripts/目录），禁止直接调 mcp_yfinance（429限流）或 curl Yahoo Finance API。** 宏观数据（VIX/DXY/TNX）不足时用 Tavily 搜索补充。

## 路由流程

1. 读取 [references/routing.md](./references/routing.md)。
2. 运行 `python3 scripts/route_market.py "<user request or symbol>"` 做第一轮分类。
3. 根据分类结果转入对应 skill：
