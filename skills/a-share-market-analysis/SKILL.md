...
2. 再给 `方向质量门槛`：列出技术、北向/资金、广度、板块、宏观、个股/量化等维度是偏多/偏空/中性/缺失。
3. 必须做 `反向审计`：若资金、广度、板块或政策新闻与技术方向冲突，最终方向降为震荡或保守档。
4. 再给 `缠论确认`，说明 CZSC 是确认、冲突还是不足。
5. A股受 T+1、涨跌停、政策与板块轮动约束，CZSC 不能覆盖这些限制。
6. 使用 `python3 -m hermes_finance analyze a-share --stock <CODE>` 或 MCP `analyze_a_share`，默认会尽量用采集器 K 线跑 CZSC。
...
### 3. 数据缺口必须降级处理
- 北向资金、涨跌家数、板块轮动三项里，只要缺两项，最终方向不得给强烈结论，只能 `偏多 / 偏空 / 震荡` 中的保守档
