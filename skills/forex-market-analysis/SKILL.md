...
- 只有技术结构、美元/利差、央行/利率、风险情绪、交叉验证中多数同向，且反向审计不过关时，才允许输出做多/做空。
...
3. 必须做 `反向审计`：若美元、利差、央行路径或风险情绪与技术方向冲突，最终方向降级为观望。
4. 再给 `缠论确认`，说明 CZSC 是确认、冲突还是不足。
5. 最终方向不能由缠论单独决定；与利率/央行路径冲突时必须降级。
6. 使用 `python3 -m hermes_finance analyze forex <SYMBOL>` 或 MCP `analyze_forex`，默认会尽量用采集器 K 线跑 CZSC。
