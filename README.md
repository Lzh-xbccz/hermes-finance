...
[![Version](https://img.shields.io/badge/version-v1.3.1-green.svg)](https://github.com/Lzh-xbccz/hermes-finance/releases)
...
它不是单一指标提示器，而是一套分层证据审计流程：行情、合约、链上、宏观、情绪、新闻、交易所价差、缠论确认分层处理。脚本只采集各维度证据，方向由 AI 基于证据逻辑强度综合判断（不用投票计数或权重打分）；证据不足或冲突时输出观望，证据充分时再给偏多/偏空判断、反向审计和交易计划。
...
- 七维证据采集 + 缠论确认，避免单一技术指标主导结论
- 各维度证据综合判断（不用投票计数或权重打分）、反向审计、情景推演和交易计划
...
每个维度独立给出偏多/偏空/中性的证据判断。同类的数据源合并成一个维度，不会重复计入。脚本只负责把证据摆出来，方向不由投票计数决定。

### 第五步：AI 综合判断方向

证据采集完，由 AI 基于各维度证据的逻辑强度综合判断方向，而不是数票数：

1. **证据充分性**——关键维度（如 K 线）缺失时不给方向
2. **证据一致性**——多维度同向且无硬反证时，给偏多/偏空
3. **硬反证优先**——比如美元暴涨 + 美股暴跌同时出现，方向打架就观望
...
过了证据判断的给方向，证据不够的就观望。观望不是失败——证据不够还硬给方向才是问题。

输出的报告里有：

- 数据完整性——哪些拉到了，哪些缺了
- 各维度证据——每个维度偏多还是偏空
- 方向判断依据——证据充分性、一致性、硬反证逐项列出来
- 反向审计——做多之前先看做空的最强理由是什么
- 缠论确认——共振还是背驰还是不可用
- 最终方向——偏多、偏空、震荡、观望
- 情景推演——如果涨了怎么办，如果跌了怎么办
- 交易计划——证据充分时才给入场/止损/止盈
...
**v1.3.1** — 全市场移除投票决策：脚本只采集各维度证据，方向由 AI 基于证据逻辑强度综合判断（不用投票计数或权重打分）

**v1.3.0** — Crypto 移除投票决策：脚本只采集各维度证据，方向由 AI 综合判断

**v1.2.13** — Crypto 趋势矩阵补强：扩散震荡遇到主高点后回调时保留父级上轨延伸，真正下降反转仍走峰值后下降链

**v1.2.12** — Crypto 形态图轨道改为直线延伸：父级上轨不再被 6/16 后 lower-high 折下去，回调链作为子趋势绘制
...
**v1.1.x** — Crypto 新闻基本面、反向审计、AI 客户端适配
