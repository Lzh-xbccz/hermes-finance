...
def _analysis_contract(result: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        "3. 方向质量门槛（逐项列出 1-7 维是偏多/偏空/中性/缺失；只按独立维度计票，不得把同类代理拆成多票；只有同一方向至少 3 个独立维度支持、领先反向至少 2 个维度、关键维度无缺口、没有硬反证时，才允许给做多/做空）",
        "4. 反向审计（先写出最强反方向证据；若反方向证据同等级或关键驱动冲突，最终方向必须降级为观望/震荡/无方向优势）",
        "5. 缠论确认（第 8 维，只做确认/冲突/不足；不得把 CZSC score 当最终方向）",
        "6. 最终方向（必须由七维主判断通过质量门槛后得出；观望/震荡/无方向优势是合法且优先的结论）",
        "7. 因果叙事",
        "8. 八维深挖：" + "、".join(dimensions),
        "9. 情景推演（方向性情景不强制合计 100%；证据不足时先给触发条件）",
        "10. 交易计划和失效条件（只有通过方向质量门槛才给主动交易计划；否则只给等待触发条件）",
        "",
        f"- Strictness: {czsc_line}",
        "- Decision rule: final direction must be driven by dimensions 1-7 and pass the direction quality gate. Do not force a directional trade; if evidence is mixed, stale, incomplete, or contradicted, output 观望/震荡/无方向优势.",
